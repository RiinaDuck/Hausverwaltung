-- Fix: zahlungen Tabelle komplett neu erstellen
-- Ausführen im Supabase SQL Editor
-- Löscht die alte Tabelle und erstellt sie mit korrekten Constraints

-- Alte Tabelle entfernen
DROP TABLE IF EXISTS zahlungen CASCADE;

-- Tabelle neu erstellen mit korrekten Constraints
CREATE TABLE zahlungen (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,

  -- mieter_id ist nullable (CAMT-Einträge können nicht zugeordnet sein)
  -- KEIN Foreign Key, da Mieter-IDs aus dem Client kommen können
  mieter_id TEXT,
